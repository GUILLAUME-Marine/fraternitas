...
const links = [
  ...
  { href: "/dashboard/spiritual", icon: "✝︎", label: "Prières" }, // ← renommé
  { section: "Personnel" },
  ...
  return (
    <aside className="hidden md:flex flex-col fixed top-16 left-0 bottom-0 w-60 bg-white border-r border-[rgba(17,16,9,0.08)] p-4 overflow-y-auto z-40">
      {links.map((link, i) => {
        ...
        const badgeCount = getBadge((link as { badge?: string }).badge);
        ...
        return (
          <Link key={link.href} href={link.href!}
            ...
            <span className={cn(
              ...
              isPremium ? "bg-[rgba(184,151,58,0.15)]" : isActive ? "bg-[rgba(255,255,255,0.15)]" : "bg-[#F7F3EC]"
            )}>
              {link.icon}
            </span>
