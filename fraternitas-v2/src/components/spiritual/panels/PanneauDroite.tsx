"use client";
// ─── panels/PanneauDroite.tsx v3 ──────────────────────────────────────────────
// Panneau droite desktop. Mobile → MobilePrayerLayout (pavé Intentions).
// ─────────────────────────────────────────────────────────────────────────────
...
export default function PanneauDroite({
  ...
  useEffect(() => {
    ...
    fetch("/api/intentions?filter=all&page=1")
      ...
      .then(d => { if (d?.intentions) setIntentions(d.intentions.slice(0,5)); })
      .catch(() => {})
