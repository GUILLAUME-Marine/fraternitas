"use client";
// ─── pages/ChapeletPage.tsx v5 ────────────────────────────────────────────────
// Étapes officielles du chapelet catholique :
//   INTRO : Signe de croix → Credo → Notre Père → 3×Ave (vertus) → Gloire
//   DIZAINE ×5 : Mystère+NP → 10×Ave → Gloire → Fatima
//   FIN : Salve Regina
// Écran d'accueil simplifié : plus de texte superflu, 1 seul bouton doré
// ─────────────────────────────────────────────────────────────────────────────

import { useState, useEffect, useRef, useCallback } from "react";
import { MYSTERIES, PRAYERS_TEXT, INTRO_STEPS, type IntroStep } from "../data";
import { type RosaryProgress } from "../hooks";

// ── Types de phase ─────────────────────────────────────────────────────────────
// "intro"   = étapes d'introduction (Credo, 3 Ave, etc.)
// "decade"  = prière grain par grain dans une dizaine
// "mystery" = page contempler le mystère
// "pourquoi"= page pourquoi prier le chapelet
// "done"    = chapelet terminé
type Phase = "accueil" | "setup" | "intro" | "decade" | "mystery" | "pourquoi" | "done";
type SoundMode = "silence" | "chant" | "organ";

// grainIndex dans une dizaine :
//   -1  = Notre Père (grande boule)
//   0-9 = 10× Ave Maria (petites boules)
//   10  = Gloire au Père
//   11  = Prière de Fatima (optionnelle, puis passe à la dizaine suivante)
...
function buildRosarySVG(current: number): string {
  ...
  const start=Math.PI/2+Math.PI*0.12, end=Math.PI/2-Math.PI*0.12+Math.PI*2, step=(end-start)/(total-1);
  ...
  for(let i=0;i<total;i++){
    ...
    const big=i%11===0,done=i<cur,isCur=i===cur,r=isCur?(big?13:10):(big?9:6);
...
// ── SVG chapelet compact (mobile) — vraie ellipse ─────────────────────────────
function buildMobileRosarySVG(current: number): string {
  const W=320,H=170,cx=160,cy=72,rx=138,ry=54,total=55;
  const cur=Math.max(0,Math.min(total,current));
  const start=Math.PI/2+Math.PI*0.12, end=Math.PI/2-Math.PI*0.12+Math.PI*2, step=(end-start)/(total-1);
  ...
  for(let i=0;i<total;i++){
    const a=start+i*step, x=cx+rx*Math.cos(a), y=cy+ry*Math.sin(a);
    const big=i%11===0,done=i<cur,isCur=i===cur,r=isCur?(big?8:6):(big?5.5:3.5);
    const fill=done?"#7A551D":isCur?"#C49A3C":"#E2D8CC";
    s+=`<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}" fill="${fill}" stroke="${isCur?"rgba(184,137,58,.50)":"rgba(17,16,9,.12)"}" stroke-width="${isCur?3.5:0.8}"/>`;
  }
...
function IntroSVG() {
  ...
  const start=Math.PI/2+Math.PI*0.12, end=Math.PI/2-Math.PI*0.12+Math.PI*2, step=(end-start)/54;
  let circles="";
  for(let i=0;i<55;i++){
    const a=start+i*step, x=cx+rx*Math.cos(a), y=cy+ry*Math.sin(a);
    circles+=`<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${i%11===0?9:5.5}" fill="${i===0?"#C49A3C":"#E9DFD1"}" stroke="rgba(17,16,9,.12)"/>`;
  }
  return (<svg viewBox="0 0 560 300" aria-hidden="true">
...
// ── Bouton Amen partagé ────────────────────────────────────────────────────────
function AmenBtn({label,onTap,tap,minH=120}:{label:string;onTap:()=>void;tap:boolean;minH?:number}) {
  return (
    <button className={`amen-btn${tap?" tap":""}`} onClick={onTap} style={{minHeight:minH}}>
      ...
      <span className="amen-text">{label}</span>
    </button>
  );
}
...
export default function ChapeletPage({
  ...
  // Étape d'intro en cours (index dans INTRO_STEPS)
  const [introIndex, setIntroIndex] = useState(0);
  // Note visible dans l'intro
  ...
  // ── Prière courante dans une dizaine ──────────────────────────────────────
  const getCurrentPrayer = (): {label:string;text:string;sublabel?:string} => {
    ...
    if (gi === 10)          return {label:"Gloire au Père", text:PRAYERS_TEXT.GL};
    return {label:"Prière de Fatima", text:PRAYERS_TEXT.FATIMA, sublabel:"Traditionnelle après chaque dizaine"};
  };
  ...
  const startSound = useCallback(() => {
    ...
    stopSound();
    try {
      const audio = new Audio(src); audio.loop=true; audio.volume=0;
      audio.play().then(()=>{
        let v=0; const fade=setInterval(()=>{v=Math.min(v+0.04,0.45);audio.volume=v;if(v>=0.45)clearInterval(fade);},200);
      }).catch(()=>{});
  ...
  const advanceGrain = () => {
    ...
    else if (gi === 9) { p.grainIndex = 10; }  // Gloire
    else if (gi === 10){ p.grainIndex = 11; }  // Fatima
    else if (gi === 11) {
      // Fin de cette dizaine
      if (p.decadeIndex < 4) {
        p.decadeIndex++; p.grainIndex = -1;    // Dizaine suivante
      } else {
        // Fin du chapelet → Salve Regina
        stopSound(); onClear(); onCompleted?.(); setPhase("done"); return;
      }
    }
    save(p);
  ...
  const startPraying = () => {
    ...
    setIntroIndex(0);
    setIntroNoteOpen(false);
    // Si reprise → on saute l'intro et on va directement dans la dizaine
  ...
  const pause = () => {
    save(localProgress); stopSound(); onClose();
    setTimeout(()=>window.scrollTo({top:0,behavior:"smooth"}), 50);
  };

  const introStep = INTRO_STEPS[introIndex];
  const {label:prayLabel, text:prayText, sublabel} = getCurrentPrayer();
  ...
  // ── Label du bouton Amen selon la position ────────────────────────────────
  const amenLabel = (): string => {
    const gi = localProgress.grainIndex;
    if (gi === 10) return "Gloire au Père — Amen";
    if (gi === 11) {
      return localProgress.decadeIndex < 4 ? "Dizaine suivante →" : "Terminer le chapelet →";
    }
    return "Amen";
  };

  // ── Composant prière partagé (mobile + desktop) ───────────────────────────
  const PrayerContent = ({mobile}:{mobile:boolean}) => (
    <>
      {localProgress.intention && (
        <div style={{background:"rgba(255,255,255,.76)",border:"1px solid var(--gold-b)",borderRadius:mobile?12:20,padding:mobile?"9px 13px":"12px 16px"}}>
          <p style={{fontSize:13,fontWeight:300,color:"var(--ink65)",fontStyle:"italic"}}>🙏 {localProgress.intention}</p>
        </div>
      )}

      {sublabel && (
        <p style={{fontSize:11,fontWeight:600,letterSpacing:".10em",textTransform:"uppercase",color:"var(--gold)",textAlign:"center"}}>{sublabel}</p>
      )}

      <div style={{background:"#FFFDF8",border:"1px solid rgba(17,16,9,.10)",borderRadius:mobile?16:22,padding:mobile?"14px 16px":"18px 20px",display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
        <span style={{fontFamily:"var(--serif)",fontSize:mobile?20:24,fontWeight:300,letterSpacing:"-.03em",color:"var(--ink)",lineHeight:1.1,flex:1}}>
          {prayLabel}
        </span>
      ...
      {prayTextOpen && (
        <div style={{background:"#FFFDF8",border:"1px solid rgba(17,16,9,.10)",borderRadius:mobile?16:20,padding:mobile?16:18,fontFamily:"var(--serif)",fontSize:mobile?16:16,fontWeight:300,lineHeight:1.72,color:"var(--ink80)",whiteSpace:"pre-line"}}>
          {prayText}
        </div>
      )}

      <AmenBtn label={amenLabel()} onTap={advanceGrain} tap={amenTap} minH={mobile?100:120}/>
      <p style={{fontFamily:"var(--serif)",fontSize:13,fontStyle:"italic",color:"var(--ink50)",textAlign:"center"}}>L&apos;Église prie avec vous</p>
    </>
  );
  ...
  return (
    <div className="chap-page visible" id="chap-page">
      ...
      {phase==="accueil" && (
        <div className="chap-intent">
          ...
          <div className="chap-inner">
            ...
            {/* Card simplifiée — 1 seul bouton doré */}
            <div className="rosary-start-card">
              <p style={{fontSize:10,fontWeight:700,letterSpacing:".12em",textTransform:"uppercase",color:"var(--gold)",marginBottom:6}}>
                {mystery?.label} · {mystery?.days}
              </p>
              {hasProgress && percent > 0 && (
                <>
                  <p style={{fontSize:13,color:"var(--ink65)",marginBottom:10}}>{humanPosition()} · {percent}% avancé</p>
                  <div className="chap-progress-track"><div className="chap-progress-fill" style={{width:`${percent}%`}}/></div>
                </>
              )}
              <div className="rosary-actions" style={{marginTop:hasProgress?0:8}}>
                {/* Bouton doré unique */}
      ...
      {/* ═══ INTRO OFFICIELLE ═══ */}
      {/* Signe de croix, Credo, Notre Père, 3× Ave, Gloire */}
      {phase==="intro" && introStep && (
        <div className="chap-intent">
          <header className="chap-hdr">
            <BackBtn onClick={pause} label="Pause"/>
            <span className="chap-hdr-title">Introduction</span>
            <span style={{fontSize:11,fontWeight:600,color:"var(--ink50)"}}>
              {introIndex+1}/{INTRO_STEPS.length}
            </span>
          </header>
          <div className="chap-inner" style={{maxWidth:640}}>
            {/* Carte étape intro */}
            <div style={{background:"#FFFDF8",border:"1px solid rgba(17,16,9,.10)",borderRadius:24,overflow:"hidden",boxShadow:"var(--sh)"}}>
              <div style={{padding:"14px 20px",borderBottom:"1px solid var(--ink12)",background:"var(--cream3)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div>
                  <p style={{fontSize:10,fontWeight:700,letterSpacing:".12em",textTransform:"uppercase",color:"var(--gold)",marginBottom:2}}>
                    Introduction · étape {introIndex+1}
                  </p>
                  <p style={{fontFamily:"var(--serif)",fontSize:20,fontWeight:400,color:"var(--ink)",lineHeight:1.1}}>
                    {introStep.label}
                  </p>
                </div>
              </div>
              <div style={{padding:"22px 22px"}}>
                <button onClick={()=>setPrayText(v=>!v)} style={{fontSize:12,fontWeight:700,color:"var(--gold)",background:"#FFF8EC",border:"1px solid var(--gold-b)",borderRadius:99,padding:"6px 12px",cursor:"pointer",marginBottom:14}}>
                  {prayTextOpen?"Masquer le texte":"Lire le texte"}
                </button>
                {prayTextOpen && (
                  <p style={{fontFamily:"var(--serif)",fontSize:isMobile?16:18,fontWeight:300,fontStyle:"italic",color:"var(--ink)",lineHeight:1.72,whiteSpace:"pre-line",marginBottom:14}}>
                    {introStep.text}
                  </p>
                )}
                {introStep.note && (
                  <>
                    <button onClick={()=>setIntroNoteOpen(v=>!v)} style={{fontSize:11,fontWeight:600,color:"var(--gold)",background:"var(--gold-pale)",border:"1px solid var(--gold-b)",borderRadius:99,padding:"4px 10px",cursor:"pointer"}}>
                      {introNoteOpen?"Masquer":"Comment faire ?"}
                    </button>
                    {introNoteOpen && (
                      <p style={{fontSize:12,color:"var(--ink50)",lineHeight:1.55,marginTop:8}}>
                        {introStep.note}
                      </p>
                    )}
                  </>
            ...
            <div style={{display:"flex",gap:5,justifyContent:"center"}}>
              {INTRO_STEPS.map((_,i)=>(
                <div key={i} style={{height:5,borderRadius:99,background:i<introIndex?"var(--gold2)":i===introIndex?"var(--gold)":"rgba(17,16,9,.10)",width:i===introIndex?20:8,transition:"all .3s"}}/>
              ))}
            </div>

            <AmenBtn label="Amen →" onTap={advanceIntro} tap={amenTap}/>
        ...
        <div className="chap-pray visible">
          ...
          {isMobile ? (
            <div style={{display:"flex",flexDirection:"column",gap:10,padding:"12px 14px",paddingBottom:"calc(14px + env(safe-area-inset-bottom,0px))"}}>
              {/* Mystère compact */}
              <button onClick={()=>setPhase("mystery")} style={{width:"100%",background:"linear-gradient(145deg,#2C1E08,#1A1410)",border:"1px solid rgba(196,154,60,.24)",borderRadius:16,padding:"12px 14px",textAlign:"left",cursor:"pointer"}}>
                ...
                <p style={{fontFamily:"var(--serif)",fontSize:18,fontWeight:300,color:"#FFF8EA",lineHeight:1.1,marginBottom:4}}>
                  {mysteryItem?.title}
                </p>
                <p style={{fontFamily:"var(--serif)",fontSize:12,fontStyle:"italic",color:"rgba(255,248,234,.65)"}}>
                  Contempler →
                </p>
              </button>

              {/* SVG ellipse compact */}
              <div style={{background:"#FFFDF8",border:"1px solid rgba(17,16,9,.08)",borderRadius:14,padding:"10px 8px",display:"flex",alignItems:"center",justifyContent:"center"}}>
                <div dangerouslySetInnerHTML={{__html:mobileSvgHtml}} style={{width:"100%"}}/>
              </div>

              <PrayerContent mobile={true}/>
            </div>
      ...
      {phase==="done" && (
        <div className="chap-pray visible">
          ...
          <div style={{maxWidth:460,margin:"0 auto",padding:"40px 22px 80px",display:"flex",flexDirection:"column",alignItems:"center",gap:16,textAlign:"center"}}>
            ...
            <h2 style={{fontFamily:"var(--serif)",fontSize:28,fontWeight:300,letterSpacing:"-.04em",color:"var(--ink)"}}>Chapelet terminé.</h2>
            <p style={{fontSize:13,color:"var(--ink50)",lineHeight:1.65,maxWidth:320}}>Prenez un moment de silence. L&apos;Église a prié avec vous.</p>

            {/* Salve Regina */}
            <div style={{background:"var(--cream3)",borderRadius:16,padding:18,border:"1.5px solid var(--gold-b)",width:"100%",textAlign:"left"}}>
              <p style={{fontSize:10,fontWeight:700,letterSpacing:".10em",textTransform:"uppercase",color:"var(--gold)",marginBottom:8}}>Salve Regina</p>
              <p style={{fontFamily:"var(--serif)",fontSize:15,fontWeight:300,fontStyle:"italic",color:"var(--ink80)",lineHeight:1.68,whiteSpace:"pre-line"}}>
                {PRAYERS_TEXT.SALVE}
              </p>
            </div>

            {/* Prière de Fatima */}
            <div style={{background:"var(--paper)",borderRadius:14,padding:16,border:"1px solid var(--ink12)",width:"100%",textAlign:"left"}}>
              ...
              <p style={{fontFamily:"var(--serif)",fontSize:14,fontWeight:300,fontStyle:"italic",color:"var(--ink80)",lineHeight:1.68,whiteSpace:"pre-line"}}>
                {PRAYERS_TEXT.FATIMA}
              </p>
            </div>
      ...
      {phase==="mystery" && (
        <div className="mystery-detail-page visible">
          <header className="chap-hdr">
            <BackBtn onClick={()=>setPhase("decade")}/>
          ...
          <div className="aide-inner">
            ...
            <button className="btn-primary" onClick={()=>setPhase("decade")}>Revenir au chapelet →</button>
          </div>
        </div>
      )}
      ...
      {phase==="pourquoi" && (
        <div className="rosary-info-page visible">
          ...
          <div className="aide-inner" style={{maxWidth:760}}>
            ...
            <div className="ri-card"><h3>Structure officielle</h3>
              <p>Le chapelet catholique commence par le Signe de croix, puis le Je crois en Dieu, un Notre Père, trois Je vous salue Marie pour les vertus théologales, et un Gloire au Père. Ensuite viennent cinq dizaines, chacune précédée de l&apos;annonce du mystère et d&apos;un Notre Père, suivie de dix Ave Maria et d&apos;un Gloire au Père. Il se conclut par la Salve Regina.</p>
            </div>
            ...
            <button className="btn-primary" onClick={()=>setPhase("accueil")}>Retour →</button>
          </div>
        </div>
      )}
    </div>
