"use client";
// ─── pages/ChapeletPage.tsx v3 ────────────────────────────────────────────────
// Fix mobile : tout visible sans scroller sur téléphone.
//
// Problème v2 : la prière était ouverte par défaut (~300px), le bouton Amen
// tombait hors écran. Sur mobile : prayTextOpen = false par défaut.
// Scroll uniquement si l'utilisateur clique "Lire" — c'est intentionnel.
//
// Architecture mobile phase "praying" :
//   Header 60px sticky
//   + Bandeau mystère compact (~120px sur mobile)
//   + Nom de la prière + bouton Lire (~72px)
//   + Bouton Amen (120px fixe)
//   + "L'Église prie avec vous" (30px)
//   = ~402px → tient dans tous les écrans ≥ 390px
//
// Le SVG chapelet est masqué sur mobile (rosary-visual-wrap display:none).
// ─────────────────────────────────────────────────────────────────────────────
...
function buildRosarySVG(current: number): string {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const sw  = isCur ? 5 : 1;
    const st  = isCur ? "rgba(184,137,58,.45)" : "rgba(17,16,9,.10)";
    s += `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}" fill="${fill}" stroke="${st}" stroke-width="${sw}"/>`;
  }
  return s;
}

// ── SVG chapelet compact pour mobile ─────────────────────────────────────────
// Barre horizontale de progression avec 55 grains sur une seule ligne

function buildCompactRosarySVG(current: number): string {
  const W = 320, H = 32, total = 55;
  const cur = Math.max(0, Math.min(total, current));
  // Grains sur une seule ligne
  const spacing = (W - 16) / (total - 1);
  let s = "";
  for (let i = 0; i < total; i++) {
    const x    = 8 + i * spacing;
    const y    = 16;
    const big  = i % 11 === 0;
    const done = i < cur, isCur = i === cur;
    const r    = isCur ? (big ? 8 : 6) : (big ? 5 : 3.5);
    const fill = done ? "#7A551D" : isCur ? "#C49A3C" : "#DDD4C8";
    const sw   = isCur ? 3 : 1;
    const st   = isCur ? "rgba(184,137,58,.50)" : "rgba(17,16,9,.12)";
    s += `<circle cx="${x.toFixed(1)}" cy="${y}" r="${r}" fill="${fill}" stroke="${st}" stroke-width="${sw}"/>`;
  }
  return `<svg viewBox="0 0 ${W} ${H}" aria-hidden="true" style="width:100%;height:32px">${s}</svg>`;
}
...
export default function ChapeletPage({
  ...
  // Fix mobile : texte masqué par défaut sur mobile, visible sur desktop
  const [prayTextOpen, setPrayText] = useState(false);

  useEffect(() => {
    const mobile = isMobileViewport();
    setIsMobile(mobile);
    // Sur desktop, on ouvre le texte par défaut
    ...
    try {
      ...
      audio.play().then(() => {
        ...
        const fade = setInterval(() => {
          v = Math.min(v + 0.04, 0.45); audio.volume = v;
  ...
  // Avancer grain — ne touche PAS prayTextOpen (Fix #9)
  const advanceGrain = () => {
    setAmenTap(true);
    ...
    save(p);
    // Sur mobile : on referme le texte quand on avance (le prochain grain commence masqué)
    // Sur desktop : on garde la préférence de l'utilisateur
  ...
  const startPraying = () => {
    ...
    startSound();
    setPhase("praying");
    // Mobile : commence masqué. Desktop : commence ouvert.
    setPrayText(!isMobile);
  };

  const pause = () => { save(localProgress); stopSound(); onClose(); };
  ...
  const compactSvgHtml = buildCompactRosarySVG(localProgress.decadeIndex * 11 + (localProgress.grainIndex + 1));

  // ── Bandeau mystère compact pour mobile ───────────────────────────────────
  const MysteryBarMobile = () => (
    <button
      onClick={() => setPhase("mystery")}
      style={{
        width: "100%", background: "linear-gradient(145deg,#2C1E08,#1A1410)",
        border: "1px solid rgba(196,154,60,.24)", borderRadius: 18,
        padding: "14px 16px", textAlign: "left", cursor: "pointer",
      }}
    >
      <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".12em", textTransform: "uppercase", color: "rgba(217,181,91,.90)", marginBottom: 4, fontFamily: "var(--sans)" }}>
        {localProgress.decadeIndex + 1}e mystère · {mysteryItem?.ref}
      </p>
      <p style={{ fontFamily: "var(--serif)", fontSize: 20, fontWeight: 300, color: "#FFF8EA", lineHeight: 1.1, marginBottom: 6 }}>
        {mysteryItem?.title}
      </p>
      {/* SVG compact horizontal */}
      <div dangerouslySetInnerHTML={{ __html: compactSvgHtml }} style={{ marginBottom: 6 }} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 11, color: "rgba(217,181,91,.70)", fontFamily: "var(--sans)" }}>
          {localPct}% · Contempler →
        </span>
        <span style={{ fontSize: 11, color: "rgba(217,181,91,.60)", fontFamily: "var(--sans)" }}>
          {mystery?.label}
        </span>
      </div>
    </button>
  ...
  return (
    <div className="chap-page visible" id="chap-page">
      ...
      {/* ═══ ACCUEIL ═══ */}
      {phase === "accueil" && (
        <div id="chap-phase-intent" className="chap-intent">
          ...
          <div className="chap-inner">
            <div className="chap-intro-grid-final">
              <section className="chap-intro-copy">
                ...
                <p>Commencez par contempler le mystère du jour, puis choisissez votre intention et votre ambiance avant de prier grain après grain.</p>
              </section>
              <section className="chap-intro-visual">
                <IntroRosarySVG />
                <p className="chap-fatima">
                  « Priez le chapelet chaque jour. »
                  <span>Notre-Dame à Fatima</span>
                </p>
              </section>
            </div>
            ...
            <div className="rosary-start-card">
              ...
              <p>
                {hasProgress && percent > 0
                  ? `${humanPosition()} · ${percent}% avancé.`
                  : `${mystery?.label ?? ""} · ${mystery?.days ?? ""}.`}
              </p>
              {hasProgress && percent > 0 && (
                <div className="chap-progress-track">
                  <div className="chap-progress-fill" style={{ width: `${percent}%` }} />
                </div>
              )}
      ...
      {phase === "setup" && (
        <div id="chap-phase-intent" className="chap-intent">
          <header className="chap-hdr">
            <button className="chap-back" onClick={() => setPhase("accueil")}>
              <svg width="14" height="14" fill="none" viewBox="0 0 16 16">
                <path d="M10 12L6 8l4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
              Retour
          ...
          <div className="chap-inner">
            <div className="chap-setup" style={{ display: "flex" }}>
              ...
              <textarea
                className="int-ta" rows={2}
                placeholder="Pour mon père malade… Pour la paix en Europe…"
                maxLength={200} value={intentionInput}
              ...
              <div className="son-block">
                ...
                <div className="son-btns">
                  {(["silence", "chant", "organ"] as SoundMode[]).map(mode => (
                    <button key={mode}
                      className={`son-btn${soundMode === mode ? " active" : ""}`}
              ...
              <button className="btn-primary" onClick={startPraying}>
                Entrer en prière →
              </button>
            </div>
          </div>
      ...
      {phase === "praying" && (
        <div id="chap-phase-pray" className="chap-pray visible">
          <header className="chap-hdr">
            <button className="chap-back" onClick={pause}>
              <svg width="14" height="14" fill="none" viewBox="0 0 16 16">
          ...
          {/* ── LAYOUT MOBILE : tout tient sans scroll ── */}
          {isMobile ? (
            <div style={{
              display: "flex", flexDirection: "column", gap: 10,
              padding: "14px 16px",
              // Padding bas = safe area + MobileNav globale (60px)
              paddingBottom: "calc(16px + env(safe-area-inset-bottom, 0px))",
            }}>
              {/* Mystère compact */}
              <MysteryBarMobile />

              {/* Intention si renseignée */}
              {localProgress.intention && (
                <div style={{
                  background: "rgba(255,255,255,.76)", border: "1px solid var(--gold-b)",
                  borderRadius: 14, padding: "10px 14px",
                }}>
                  <p style={{ fontSize: 13, fontWeight: 300, color: "var(--ink65)", fontStyle: "italic" }}>
                    🙏 {localProgress.intention}
                  </p>
                </div>
              )}

              {/* Nom de la prière + bouton Lire */}
              <div style={{
                background: "#FFFDF8", border: "1px solid rgba(17,16,9,.10)",
                ...
                <span style={{
                  ...
                  letterSpacing: "-.03em", color: "var(--ink)", lineHeight: 1.1, flex: 1,
                }}>
                  {prayLabel}
                </span>
                <button
                  onClick={() => setPrayText(v => !v)}
                  style={{
                    fontSize: 12, fontWeight: 700, color: "var(--gold)",
                    background: "#FFF8EC", border: "1px solid var(--gold-b)",
                    borderRadius: 99, padding: "7px 13px", cursor: "pointer", flexShrink: 0,
                  }}
                >
              ...
              {/* Texte prière — visible seulement si ouvert */}
              {prayTextOpen && (
                <div style={{
                  background: "#FFFDF8", border: "1px solid rgba(17,16,9,.10)",
                  borderRadius: 18, padding: 18,
                  fontFamily: "var(--serif)", fontSize: 16, fontWeight: 300,
                  lineHeight: 1.72, color: "var(--ink80)", whiteSpace: "pre-line",
                }}>
                  {prayText}
                </div>
              )}

              {/* Bouton Amen — toujours visible */}
              <button
                className={`amen-btn${amenTap ? " tap" : ""}`}
                onClick={advanceGrain}
                style={{ minHeight: 100 }} // légèrement réduit sur mobile vs 120px desktop
              >
                ...
                <span className="amen-text">
                  {localProgress.grainIndex === 10 ? "Amen — mystère suivant" : "Amen"}
                </span>
              </button>

              <p style={{
                fontFamily: "var(--serif)", fontSize: 13, fontStyle: "italic",
                color: "var(--ink50)", textAlign: "center",
              }}>
              ...
              <div className="chap-pray-inner">
                ...
                <div className="chap-mystery-bar">
                  ...
                  >
                    <div className="cmb-inner">
                      <p className="cmb-label">
                        {localProgress.decadeIndex + 1}e mystère {mystery?.label.split(" ")[1]?.toLowerCase() ?? ""} · {mysteryItem?.ref}
                      </p>
                      <p className="cmb-title">{mysteryItem?.title}</p>
                      <p className="cmb-verse">{mysteryItem?.verse}</p>
                      <div className="chap-progress-track">
                        <div className="chap-progress-fill" style={{ width: `${localPct}%` }} />
                      </div>
                    </div>
                    <div className="contempler-btn">
                      <span>→ Contempler ce mystère</span>
                      <svg width="14" height="14" fill="none" viewBox="0 0 16 16">
                        <path d="M6 4l4 4-4 4" stroke="#C49A3C" strokeWidth="1.5" strokeLinecap="round" />
                      </svg>
                    </div>
                  </button>
                </div>
                ...
                <div className="chap-pray-main">
                  <div className="chap-svg-wrap">
                    <svg
                      viewBox="0 0 560 235" aria-hidden="true"
                      dangerouslySetInnerHTML={{ __html: svgHtml }}
                      style={{ width: "min(100%, 560px)", height: "auto", maxHeight: 300 }}
                    />
                  </div>

                  <div className="chap-prayer-actions">
                    {localProgress.intention && (
                      <div id="cp-intention-display" style={{ display: "block" }}>
                        <p>🙏 <span>{localProgress.intention}</span></p>
                      </div>
                    )}

                    <div className="pray-label-block">
                      <span className="pl">{prayLabel}</span>
                      <button className="pread" onClick={() => setPrayText(v => !v)}>
                        {prayTextOpen ? "Masquer" : "Lire"}
                      </button>
                    </div>

                    <div
                      className={`pray-full-text${prayTextOpen ? " open" : ""}`}
                      style={prayTextOpen ? {} : { display: "none" }}
                    >
                      {prayText}
                    </div>
      ...
      {phase === "done" && (
        <div id="chap-phase-pray" className="chap-pray visible">
          <header className="chap-hdr">
            <div style={{ width: 64 }} />
            <span className="chap-hdr-title">Chapelet terminé</span>
            <div style={{ width: 64 }} />
          </header>
      ...
      {phase === "mystery" && (
        <div className="mystery-detail-page visible">
          <header className="chap-hdr">
            <button className="chap-back" onClick={() => setPhase("praying")}>
              <svg width="14" height="14" fill="none" viewBox="0 0 16 16">
                <path d="M10 12L6 8l4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
              Retour
          ...
          <div className="aide-inner">
            <div className="mystery-detail-card">
              <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".12em", textTransform: "uppercase", color: "rgba(196,154,60,.80)", marginBottom: 5 }}>
                {mystery?.label} · {mysteryItem?.ref}
              </p>
            ...
            <div className="meditation-card">
              <h3>À contempler</h3>
              <p>Regardez ce mystère comme une scène vivante : les personnes, les gestes, le silence. Qu&apos;est-ce que le Christ veut vous montrer aujourd&apos;hui ?</p>
            </div>
            <div className="meditation-card">
              <h3>Intention possible</h3>
              <p>Confiez à Marie une personne ou une situation liée à ce mystère. Ne cherchez pas une belle phrase : donnez simplement un nom, une inquiétude, une action de grâce.</p>
            </div>
            <button className="btn-primary" onClick={() => setPhase("praying")}>
              Revenir au chapelet →
            </button>
          </div>
        </div>
        ...
        <div className="rosary-info-page visible">
          ...
          <div className="aide-inner" style={{ maxWidth: 760 }}>
            <div className="ri-hero">
              <p className="pk">Le chapelet</p>
              <h2>Pourquoi prier le chapelet ?</h2>
              <p>Parce que c&apos;est une prière simple et régulière : elle met le cœur au rythme de l&apos;Évangile, avec Marie, quand les mots manquent.</p>
            </div>
            <div className="ri-card">
              <h3>Comment vivre ce moment ?</h3>
              <p>Choisissez une intention, contemplez le mystère, puis avancez grain après grain. Si votre esprit part ailleurs, ce n&apos;est pas grave : revenez doucement. Dieu connaît vos limites.</p>
            </div>
            <div className="ri-card">
              <h3>Qui a demandé de prier le chapelet ?</h3>
              <p>À Fatima, Marie demande de prier le chapelet chaque jour pour la paix et la conversion des cœurs. L&apos;Église a toujours encouragé cette prière contemplative.</p>
            </div>
            <div className="ri-card">
              <h3>Les promesses de Marie</h3>
              <p>La tradition rapporte quinze promesses attachées au Rosaire. Elles expriment la confiance dans l&apos;intercession de Marie, qui conduit toujours à son Fils.</p>
              <details>
                <summary>Voir les 15 promesses</summary>
                <ol className="promise-list">
                  {[
                    "Protection spéciale et grandes grâces pour ceux qui récitent le Rosaire.",
                    "Aide pour grandir dans la foi et se détourner du péché.",
                    "Croissance des vertus et confiance en Dieu.",
                    "Détachement de ce qui éloigne du Christ.",
                    "Secours maternel de Marie dans les combats spirituels.",
                    "Consolation dans les épreuves.",
                    "Lumière pour mieux connaître Jésus.",
                    "Soutien pour les âmes confiées à Marie.",
                    "Espérance du ciel et désir de sainteté.",
                    "Grâces demandées quand elles conduisent au salut.",
                    "Aide spéciale pour ceux qui font connaître le Rosaire.",
                    "Intercession de Marie et des saints.",
                    "Union plus profonde avec Jésus-Christ.",
                    "Signe d'un cœur tourné vers Dieu.",
                    "Protection maternelle de Marie.",
                  ].map((p, i) => <li key={i}>{p}</li>)}
                </ol>
              </details>
            </div>
            <button className="btn-primary" onClick={() => setPhase("accueil")}>
              Retour au chapelet →
            </button>
          </div>
        </div>
