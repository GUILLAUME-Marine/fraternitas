"use client";
// ─── pages/ChapeletPage.tsx ───────────────────────────────────────────────────
// Page chapelet plein écran — reproduit exactement :
//   openChapelet() → renderCleanStartCard() → beginRosarySetup() →
//   startPraying() → updatePrayUI() → advanceGrain() → completedRosary()
//   + showMysteryDetail() + showWhy()
// ─────────────────────────────────────────────────────────────────────────────
...
// ── SVG Chapelet complet (buildRosarySVG du HTML) ────────────────────────────
...
// ── SVG intro (introRosarySVG du HTML) ───────────────────────────────────────

function IntroRosarySVG() {
  const W = 560, H = 300, cx = 280, cy = 128, rx = 220, ry = 86;
  const start = Math.PI / 2 + Math.PI * 0.12;
  const end   = Math.PI / 2 - Math.PI * 0.12 + Math.PI * 2;
...
export default function ChapeletPage({
  progress, hasProgress, percent, humanPosition, onSave, onClear, onClose,
}: ChapeletPageProps) {
  ...
  const [prayTextOpen, setPrayText] = useState(true);
  const [amenTap, setAmenTap]       = useState(false);
  const audioRef                    = useRef<HTMLAudioElement | null>(null);

  // Données dynamiques — dérivées de progress (state réel)
  ...
  const advanceGrain = () => {
    ...
    if (gi < 9)       { p.grainIndex++; }
      ...
      else { stopSound(); onClear(); setPhase("done"); return; }
    }
    save(p);
    setPrayText(true);
  };
    ...
    <div className="chap-page visible" id="chap-page">
          ...
          <div className="chap-inner">
            ...
            <div className="chap-intro-grid-final">
              <section className="chap-intro-copy">
                ...
                <p>
                  Commencez par contempler le mystère du jour, puis choisissez votre intention
                  et votre ambiance avant de prier grain après grain.
                </p>
              </section>
            ...
            <div className="rosary-start-card">
              ...
              <p>
                {hasProgress && percent > 0
                  ? `${humanPosition()} · ${percent}% avancé. Vous pourrez modifier l'intention avant de reprendre.`
                  : `${mystery?.label ?? ""} · ${mystery?.days ?? ""}. L'intention et l'ambiance sonore viennent juste après.`}
              </p>
              {hasProgress && percent > 0 && (
      ...
      {phase === "done" && (
        <div id="chap-phase-pray" className="chap-pray visible">
          ...
          <div style={{
            maxWidth: 460, margin: "0 auto", padding: "40px 22px 80px",
            ...
            <h2 style={{
              fontFamily: "var(--serif)", fontSize: 30, fontWeight: 300,
              letterSpacing: "-.04em", color: "var(--ink)",
            }}>
              Chapelet terminé.
            </h2>
            <p style={{ fontSize: 14, color: "var(--ink50)", lineHeight: 1.65, maxWidth: 320 }}>
              Prenez un moment de silence avant de reprendre votre journée.
              L&apos;Église a prié avec vous.
            </p>
            <div style={{
              background: "var(--cream3)", borderRadius: 16, padding: 17,
              border: "1.5px solid var(--gold-b)", width: "100%", textAlign: "left",
            }}>
              <p style={{
              ...
              <p style={{
                ...
                fontStyle: "italic", color: "var(--ink80)", lineHeight: 1.68,
              }}>
                Ô mon Jésus, pardonnez-nous nos péchés,<br />
                préservez-nous du feu de l&apos;enfer,<br />
                et conduisez au Ciel toutes les âmes,<br />
                spécialement celles qui ont le plus besoin<br />
                de votre miséricorde. Amen.
              </p>
            </div>
            <button className="btn-primary" style={{ maxWidth: 300 }} onClick={onClose}>
              Terminer →
            </button>
          </div>
        </div>
      ...
      {phase === "mystery" && (
        <div className="mystery-detail-page visible" id="mystery-detail-page">
          <header className="chap-hdr">
          ...
          <div className="aide-inner">
            <div className="mystery-detail-card">
              <p style={{
                ...
                textTransform: "uppercase", color: "rgba(196,154,60,.80)", marginBottom: 5,
              }}>
            ...
            <div className="meditation-card">
              <h3>À contempler</h3>
              <p>
                Regardez ce mystère comme une scène vivante : les personnes, les gestes, le silence.
                Qu&apos;est-ce que le Christ veut vous montrer aujourd&apos;hui ?
              </p>
            </div>
            <div className="meditation-card">
              <h3>Intention possible</h3>
              <p>
                Confiez à Marie une personne ou une situation liée à ce mystère.
                Ne cherchez pas une belle phrase : donnez simplement un nom, une inquiétude, une action de grâce.
              </p>
            </div>
      ...
      {/* ═══ PHASE POURQUOI ═══ */}
      {phase === "pourquoi" && (
        <div className="rosary-info-page visible" id="rosary-info-page">
          <header className="chap-hdr">
            <button className="chap-back" onClick={() => setPhase("accueil")}>
              <svg width="14" height="14" fill="none" viewBox="0 0 16 16">
                <path d="M10 12L6 8l4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
          ...
          <div className="aide-inner" style={{ maxWidth: 760 }}>
            <div className="ri-hero">
              ...
              <p>
                Parce que c&apos;est une prière simple et régulière : elle met le cœur au rythme
                de l&apos;Évangile, avec Marie, quand les mots manquent.
              </p>
            </div>
            <div className="ri-card">
              <h3>Comment vivre ce moment ?</h3>
              <p>
                Choisissez une intention, contemplez le mystère, puis avancez grain après grain.
                Si votre esprit part ailleurs, ce n&apos;est pas grave : revenez doucement.
                Dieu connaît vos limites.
              </p>
            </div>
            <div className="ri-card">
              <h3>Qui a demandé de prier le chapelet ?</h3>
              <p>
                À Fatima, Marie demande de prier le chapelet chaque jour pour la paix et
                la conversion des cœurs. L&apos;Église a toujours encouragé cette prière contemplative.
              </p>
            </div>
            <div className="ri-card">
              <h3>Les promesses de Marie</h3>
              <p>
                La tradition rapporte quinze promesses attachées au Rosaire. Elles expriment
                la confiance dans l&apos;intercession de Marie, qui conduit toujours à son Fils.
              </p>
              <details>
