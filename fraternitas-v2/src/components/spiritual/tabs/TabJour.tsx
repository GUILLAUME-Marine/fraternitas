"use client";
// ─── tabs/TabJour.tsx v2 ──────────────────────────────────────────────────────
// Fix #5 : suppression du bloc citation noir (.citation-block)
// Affiche uniquement : évangile propre (3 lignes + lire la suite) + saint du jour
// Le titre est géré dans PrayerZone (Fix #6)
// ─────────────────────────────────────────────────────────────────────────────
      ...
      <div className="gospel-block" style={{ borderRadius: 13, marginBottom: 16 }}>
        ...
        <div className="gospel-bh">
          {loading ? (
            ...
            <span className="gospel-ref" style={{ fontSize: 13, fontWeight: 500 }}>
              {gospel.ref}
            </span>
          ) : null}
        </div>

        {/* Corps : texte évangile */}
        <div className="gospel-bb">
          {loading ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              <Skeleton w="100%" /><Skeleton w="88%" /><Skeleton w="92%" />
            </div>
          ) : gospel ? (
