// ─── data.ts ──────────────────────────────────────────────────────────────────
// Source de vérité unique pour toutes les données de prière.
// Transcription exacte du HTML fraternitas-v3-consolidated.html — rien n'est inventé.
// ─────────────────────────────────────────────────────────────────────────────
...
export interface PrayerStep {
  ...
  title: string;
  text: string;
  note: string | null; // "aide|..." = bouton toggle ; sinon paragraphe direct
}
...
export interface AideQ1Item {
  id: string;
  label: string;
  hasFollowup: boolean;
}

export interface AideFollowup {
  q: string;
  choices: string[];
}

export interface AidePrayer {
  title: string;
  text: string;
}
...
export const SEQUENCES: Record<string, Sequence> = {
  matin: {
    ...
    steps: [
      ...
      {
        num: "4", title: "L'Angélus (si c'est l'heure)",
        ...
        note: "L'Angélus se prie à 6h, 12h et 18h. Le matin, vous pouvez le remplacer par un Je vous salue Marie.",
      },
      {
  ...
  soir: {
    ...
    steps: [
      ...
      },
      {
        ...
        note: "Prenez 2 à 3 minutes de silence pour relire votre journée concrètement.",
      },
      {
...
export const MYSTERIES: Record<string, Mystery> = {
  joyeux: {
    label: "Mystères Joyeux", days: "Lundi · Samedi",
    items: [
  ...
  lumineux: {
    label: "Mystères Lumineux", days: "Jeudi",
    items: [
  ...
  douloureux: {
    label: "Mystères Douloureux", days: "Mardi · Vendredi",
    items: [
  ...
  glorieux: {
    label: "Mystères Glorieux", days: "Mercredi · Dimanche",
    items: [
...
// ── Textes des prières du chapelet ────────────────────────────────────────────
...
// ── Citations (rotation par jour de semaine) ──────────────────────────────────
// Note : dans la version connectée, la citation est extraite de l'évangile AELF.
// Ce tableau sert de fallback si l'API échoue.
...
// ── Pourquoi le chapelet — 15 promesses ──────────────────────────────────────

export const ROSARY_PROMISES: string[] = [
...
export function getLiturgicalLabel(): string {
  ...
  const [em, ed] = easterDates[y] ?? [4, 5];
  const easter = new Date(y, em - 1, ed);
  const cur    = new Date(y, m - 1, d);
  const diff   = Math.floor((cur.getTime() - easter.getTime()) / 86400000);
  if (diff >= 0 && diff <= 49) {
    const ords = ["1re","2e","3e","4e","5e","6e","7e"];
    if (diff === 49) return "Pentecôte";
    return ords[Math.min(Math.floor(diff / 7), 6)] + " semaine de Pâques";
  }
