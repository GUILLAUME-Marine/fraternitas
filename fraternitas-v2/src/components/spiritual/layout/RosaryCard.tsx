"use client";
// ─── layout/RosaryCard.tsx ────────────────────────────────────────────────────
// Carte chapelet du dashboard — reproduit exactement :
//   <section id="chapelet-card" class="rosary-card fu">
//     - titre mystère du jour
//     - barre de progression
//     - boutons Commencer / Changer de mystère
//     - SVG mini chapelet (drawMiniRosary)
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Mini SVG chapelet (drawMiniRosary du HTML) ────────────────────────────────
...
function drawMiniRosary(svg: SVGSVGElement, progress: RosaryProgress) {
  ...
  ellipse.setAttribute("stroke", "rgba(245,239,228,.15)");
  ...
  for (let d = 0; d < 5; d++) {
    ...
    const done   = d * 11 < current;
    const isCur  = d * 11 === current;
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const gi2   = d * 11 + i + 1;
      const done2 = gi2 < current;
      const isCur2= gi2 === current;

      const c2 = document.createElementNS(ns, "circle");
      c2.setAttribute("cx", x2.toFixed(1));
      c2.setAttribute("cy", y2.toFixed(1));
...
export default function RosaryCard({
  progress, hasProgress, percent, humanPosition, onOpen, onChangeMystery,
}: RosaryCardProps) {
  ...
  return (
    <section id="chapelet-card" className="rosary-card fu" style={{ animationDelay: ".28s" }}>
      {/* Colonne gauche : infos + boutons */}
      <div style={{ position: "relative", zIndex: 1 }}>
        <p className="rc-k">Chapelet interactif</p>
      ...
      {/* Colonne droite : SVG mini chapelet */}
