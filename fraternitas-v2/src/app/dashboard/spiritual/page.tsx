...
export default function PrionsPage() {
  ...
  // Lire le tab depuis le hash URL (pour retour depuis parcours/prière)
  ...
  // Lire le hash à l'init et lors de navigations
  useEffect(() => {
    const readHash = () => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: C.sans }}>
      ...
      {/* ══ HEADER — onglets uniquement, sobre, sans bandeau doré ══ */}
      <header style={{
      ...
      <main style={{ maxWidth: "600px", margin: "0 auto", padding: "14px 14px 80px" }}>
        ...
        {tab === "jour" && (
          <div className="fu" style={{ display: "flex", flexDirection: "column", gap: "0" }}>
            ...
            {/* ── Bandeau liturgique — UNIQUEMENT dans Du jour ── */}
            {!loading && data?.liturgicalInfo && (
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <CrossIcon size={14} color="rgba(196,154,60,0.70)" />
                  <span style={{ fontFamily: C.serif, fontSize: "15px", fontWeight: 400, color: "#F5EFE4", letterSpacing: "0.01em" }}>
                    Prions
                  </span>
                </div>
                <div style={{ flex: 1, textAlign: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>
                    <div style={{
                      ...
                      border: data.liturgicalInfo.color === "white" ? "1.5px solid rgba(255,255,255,0.5)" : "none",
                      flexShrink: 0,
                    }} />
                    <span style={{ fontFamily: C.sans, fontSize: "11px", fontWeight: 700, color: "rgba(255,255,255,0.88)", letterSpacing: "0.01em" }}>
                      {data.liturgicalInfo.week}
                    </span>
                  </div>
                </div>
                <span style={{ fontSize: "12px", fontWeight: 500, color: "rgba(255,255,255,0.55)", fontFamily: C.sans }}>
                  {todayFull}
                </span>
              </div>
            )}

            {/* Évangile — nom de l'évangéliste */}
              ...
              <div style={{ background: C.card, borderRadius: "16px", border: `1.5px solid ${C.cardBorder}`, overflow: "hidden" }}>
                {loading ? (
                  ...
                  <div style={{ padding: "14px 16px" }}>
                    ...
                    <p style={{ fontFamily: C.serif, fontSize: "16px", fontWeight: 300, fontStyle: "italic", color: C.ink80, lineHeight: 1.52, whiteSpace: "pre-line" }}>
                      {previewLines}
                    </p>
            ...
            <div style={{ marginBottom: "6px" }}>
              {loading ? (
                ...
                <div style={{ background: C.cream, borderRadius: "16px", padding: "12px 14px", border: `1.5px solid ${C.goldBorder}` }}>
                  <p style={{ fontSize: "9px", fontWeight: 500, letterSpacing: "0.12em", textTransform: "uppercase", color: C.gold, fontFamily: C.sans, marginBottom: "6px" }}>Saint du jour</p>
        ...
        {tab === "chapelet" && (
          <div className="fu" style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            <div style={{ background: "linear-gradient(135deg,#1A1410 0%,#2C1E08 100%)", borderRadius: "16px", border: "1.5px solid rgba(196,154,60,0.22)", padding: "20px 18px", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
                <svg width="200" height="200" viewBox="0 0 200 200" fill="none">
                  <circle cx="100" cy="100" r="90" stroke="#C49A3C" strokeWidth="0.5" opacity="0.10"/>
                  <circle cx="100" cy="100" r="66" stroke="#C49A3C" strokeWidth="0.4" opacity="0.07"/>
                  <circle cx="100" cy="100" r="44" stroke="#C49A3C" strokeWidth="0.4" opacity="0.05"/>
                </svg>
              </div>
            ...
            {rosary ? (
              <div style={{ background: C.card, borderRadius: "14px", padding: "14px", border: "1.5px solid rgba(196,154,60,0.35)" }}>
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  ...
                  <button onClick={() => { try { localStorage.removeItem(STORAGE_KEY); } catch {} setRosary(null); }} style={{ flex: 1, padding: "11px", borderRadius: "12px", border: `1.5px solid ${C.ink12}`, background: "none", fontSize: "12px", color: C.ink50, cursor: "pointer", fontFamily: C.sans }}>
                    Effacer
                  </button>
                </div>
              </div>
            ...
            <p style={{ fontSize: "10px", fontWeight: 300, color: C.ink35, textAlign: "center", fontFamily: C.sans }}>Progression sauvegardée automatiquement</p>
          </div>
        )}
