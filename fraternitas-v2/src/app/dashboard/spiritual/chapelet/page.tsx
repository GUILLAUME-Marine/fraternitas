...
const AUDIO_URLS = {
  ...
  chant: "https://upload.wikimedia.org/wikipedia/commons/b/b7/Gregorian_chant_-_Kyrie_IX.ogg",
};
    ...
    <>
      ...
      <main style={{ maxWidth: "540px", margin: "0 auto", padding: "4px 14px 12px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
          ...
          {/* Intention — affichée + bouton Modifier */}
          {(progress.intention || editingIntention) && (
            <div style={{ background: C.creamGold, borderRadius: "10px", padding: "8px 12px", border: `1px solid ${C.goldBorder}` }}>
              {editingIntention ? (
                <div>
                  <textarea
                    defaultValue={progress.intention}
                    rows={2}
                    autoFocus
                    id="intention-edit"
                    style={{ width: "100%", background: "none", border: "none", outline: "none", fontFamily: C.serif, fontSize: "13px", fontStyle: "italic", color: C.ink65, resize: "none", lineHeight: 1.5 }}
                  />
                  <button onClick={() => {
                    const el = document.getElementById("intention-edit") as HTMLTextAreaElement;
                    const newInt = el?.value || "";
                    setIntention(newInt);
                    doSave({ ...progress, intention: newInt });
                    setEditingIntention(false);
                  }} style={{ fontSize: "11px", fontWeight: 600, color: C.gold, background: "none", border: "none", cursor: "pointer", fontFamily: C.sans, padding: 0 }}>
                    Valider
                  </button>
                </div>
              ) : (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px" }}>
                  ...
                  <button onClick={() => setEditingIntention(true)} style={{ fontSize: "11px", fontWeight: 500, color: C.gold, background: "none", border: "none", cursor: "pointer", fontFamily: C.sans, padding: 0, flexShrink: 0 }}>
                    Modifier
                  </button>
                </div>
              )}
