...
function extractQuoteFromGospel(
  ...
  try {
    ...
    // Compatible ES2017 — pas de flag "s" (dotAll) ni de lookbehind
    const guillemetsRe = /«\s*([\s\S]+?)\s*»/g;
    let gMatch: RegExpExecArray | null;
    // eslint-disable-next-line no-cond-assign
    while ((gMatch = guillemetsRe.exec(clean)) !== null) {
      let quote = gMatch[1].replace(/\s+/g, " ").trim();
    ...
    // Fallback : première phrase forte — split ES2017 compatible (pas de lookbehind)
    const rawSentences = clean.replace(/([.!?])\s+/g, "$1\u0000").split("\u0000");
    const sentences = rawSentences
      .map((s: string) => s.trim())
      .filter((s: string) => s.length > 30 && !s.startsWith("En ce temps") && !s.startsWith("Alléluia"));
