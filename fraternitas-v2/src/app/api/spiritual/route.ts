...
// ─── Fallback citations (si évangile non disponible) ─────────────────────────
// Gardé comme sécurité — utilisé uniquement si AELF ne répond pas
...
function getFallbackQuote(dateStr: string): { text: string; author: string } {
  const date = new Date(dateStr);
  const dayOfYear = Math.floor((date.getTime() - new Date(date.getFullYear(), 0, 0).getTime()) / 86400000);
  return FALLBACK_QUOTES[dayOfYear % FALLBACK_QUOTES.length];
}

// ─── Extraction de la citation clé de l'évangile ─────────────────────────────
// Prend la PREMIÈRE parole de Jésus entre guillemets français « »
// Fallback : première phrase forte du texte
// Zéro IA, zéro hallucination — texte brut AELF
function extractQuoteFromGospel(
  gospelText: string,
  gospelRef: string,
  gospelIntro: string,
): { text: string; author: string } | null {
  if (!gospelText || gospelText.length < 30) return null;

  try {
    // Nettoyer le HTML
    const clean = gospelText
      .replace(/<br\s*\/?>/gi, " ")
      .replace(/<[^>]+>/g, " ")
    ...
    // Chercher la première citation entre guillemets français (parole de Jésus)
    const guillemetsMatches = [...clean.matchAll(/«\s*(.+?)(?:\s*»|\s*\.(?=\s[A-Z]))/gs)];

    for (const match of guillemetsMatches) {
      let quote = match[1].replace(/\s+/g, " ").trim();
      if (quote.length < 15) continue; // trop court

      // Prendre jusqu'au premier point naturel
      const dotIdx = quote.indexOf(".");
      if (dotIdx > 20 && dotIdx < 200) quote = quote.slice(0, dotIdx);
      ...
      // Attribution : nom de l'évangéliste + référence
      const evangMatch = gospelIntro.match(/selon saint\s+(\w+)/i);
      const author = evangMatch
        ? `Évangile selon saint ${evangMatch[1]} · ${gospelRef}`
        : `Évangile du jour · ${gospelRef}`;

      return { text: `« ${quote} »`, author };
    }

    // Fallback : première phrase forte (pas "En ce temps-là")
    const sentences = clean
      .split(/(?<=[.!?])\s+/)
      .map(s => s.trim())
      .filter(s => s.length > 30 && !s.startsWith("En ce temps") && !s.startsWith("Alléluia"));
...
export async function GET(req: NextRequest) {
  ...
  // La quote sera construite APRÈS avoir récupéré l'évangile
  let quote = getFallbackQuote(today);
  ...
  // ── Citation : extraite de l'évangile du jour en priorité ──────────────────
  // Zéro IA, zéro risque — texte brut AELF, authentique et différent chaque jour
  if (gospel?.text && gospel.text.length > 50) {
    const gospelQuote = extractQuoteFromGospel(
      gospel.text,
      gospel.reference,
      gospel.intro,
    );
    if (gospelQuote) quote = gospelQuote;
    // Sinon : on garde le fallback déjà défini ci-dessus
  }
