...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const total = await prisma.defunt.count(mine ? { where: { userId: session.user.id } } : undefined);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (count >= 20) return NextResponse.json({ error: "Limite de 20 défunts atteinte." }, { status: 429 });
