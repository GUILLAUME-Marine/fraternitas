...
// Et ensuite : const { id } = await params;
  ...
  try {
    const defunt = await prisma.defunt.findUnique({ where: { id: params.id } });
    ...
    const existing = await prisma.defuntPrayer.findUnique({
      where: { defuntId_userId: { defuntId: params.id, userId: session.user.id } },
    });
    ...
    await prisma.defuntPrayer.create({
      data: { defuntId: params.id, userId: session.user.id },
    });
